def proto_args(parser, std_parser, module_parser):
    ...
    ssh_parser.add_argument("--ssh-timeout", help="SSH connection timeout, default is %(default)s secondes", type=int, default=5)
    ssh_parser.add_argument("--sudo-check", action="store_true", help="Check user privilege with sudo")
    ssh_parser.add_argument("--sudo-check-method", choices={"sudo-stdin", "mkfifo"}, default="sudo-stdin", help="method to do with sudo check, default is '%(default)s'")
